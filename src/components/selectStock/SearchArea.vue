        ...
        <ul>
            <li v-for="info in queryStockThemeObj.result.list">
                <span v-if="info.infoType === 0">
                    <router-link :to="'/themeDetails?id='+info.infoId">
                ...
                <span v-if="info.infoType === 1">
                    <router-link 
                        :to="'stockDetails?code='+info.infoCode+'&name='+info.infoName+'&marketCode='+info.dataType"
                    >
...
export default {
    ...
    methods: {
        ...
        addFollow(obj){
            ...
            this.addOptionalFollowAction(obj);
        },
