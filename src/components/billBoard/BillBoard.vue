...
export default {
    data(){
        return {
            ...
            // 营业部上榜一览 
            departTop:{
                thead: ["序号", "营业部名称", "介入个股数量", "当日买入额（万）", "当日卖出额（万）", "净买额（万）"],
                tbody: [
                    // {
            ...
            // 机构上榜一览 
            mechTop:{
                thead: ["序号", "上榜股票名称", "上榜机构次数(次)", "机构席位买入金额（万）", "机构席位卖出金额（万）", "机构净买入（万）"],
                tbody: [
                    // {
    ...
    watch:{
        'week'(newVal, oldVal){
            ...
            this.topTableTitle = `营业部上榜一览(${this.year}-${this.month}-${this.day}  星期${Util.changeWeek(newVal)})`;
            this.topTableMechTitle = `机构席位上榜一览(${this.year}-${this.month}-${this.day}  星期${Util.changeWeek(newVal)})`;
    ...
    computed:{
        ...
        ...mapGetters(['mechanism', 'boardList', 'stockDetails', 'currentStockDetails', 'departmentHotList', 'billboardDays'])
    ...
    mounted(){
        ...
        this.topTableTitle = `营业部上榜一览(${year}-${month}-${day}  星期${Util.changeWeek(this.week)})`;
        this.topTableMechTitle = `机构席位上榜一览(${year}-${month}-${day}  星期${Util.changeWeek(this.week)})`;
    ...
    methods: {
        ...mapActions([
            ...
            'getMechanismList','getStockDetail_billboard']),
        //  获取选择日期
        changeDate(dateObj){
            // console.log(dateObj); 
            ...
            this.week = (new Date(`${dateObj.year}-${dateObj.month}-${dateObj.day}`)).getDay();
            ...
            // 储存 股票信息
                ...
                let stockCode = boardListItem?boardListItem[0].stockCode.split('.')[0]:null;
                // 股票详情参数
                let stockInfo = {
                ...
                data.map((item) => {
                    let obj = {
                        ...
                        rowContent: [
                            ...
                            Util.formatMoney({num: parseFloat(item.buy_sum - item.sale_sum), type:'万'})
                        ]
                    };
                ...
                data.map((item) => {
                    let obj = {
                        ...
                        rowId: {
                            ...
                            marketCode: item.secu_code.split('.')[1],
                            name: item.secu_abbr,
                        },
                        // 0 属于营业部  1 属于股票详情 
                        type: 1,
                        rowContent: [
                            ...
                            // 机构买入
                            // item.buy_sum,
                            Util.formatMoney({num: parseFloat(item.jg_buy_sum), type: '万'}),
                            // 机构卖出
                            
                            Util.formatMoney({num: parseFloat(item.jg_sale_sum), type: '万'}),
                            // 净买入
                            
                            Util.formatMoney({num: parseFloat(item.jg_net_buy_sum), type:'万'})
                        ]
                    };
