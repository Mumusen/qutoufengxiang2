...
const actions = {
    ...
    getTodayInvestFlow({commit, state}, obj){
        if(obj){
            axios({
                method: 'get',
                url: Util.url() + 'fundflow/getFundflow',
                responseType:'json'
                // params: obj
            })
            .then((res)=>{
    ...
    getStockDetails({commit, state}, obj){
        ...
        .then((res)=>{
            if(res){
                ...
                if(res.data.code){
                    // 股票基本信息
                    commit(types.STOCK_BASIC_INFO, {...res.data.detail, fellow: res.data.fellow});
                    // 个股关联主题
    ...
    getThemeUpDownCount({commit, state}, obj){
        axios({
            method: 'get',
            url: Util.url() + 'selfSelectInfo/getInfoDetails',
            responseType:'json',
    ...
    */
    ...
    getDepartmentTypeStockList({commit, state}, obj){
        axios({
            ...
            if(res){
                commit(types.DEPARTMENT_QUEY_STOCK, res.data);
            }
        });
    },
